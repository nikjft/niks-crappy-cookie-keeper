...
const CLIENT_JS_CODE = `!function(e){"use strict";if(e.nccs&&e.nccs.initialized)return;const t="_nccs_session",n={initialized:!0,isReady:!1,readyQueue:[],consent:!0,values:{},ready:function(e){if("function"==typeof e)if(this.isReady)try{e()}catch(e){console.error("[NCCS] Callback error:",e)}else this.readyQueue.push(e)},setConsent:function(e){this.consent=!!e,console.log(\`[NCCS] Consent status updated: \${this.consent}\`)},init:function(...n){let s={};const i=[];if(n.forEach(e=>{"string"==typeof e?i.push({name:e,generate:!1}):"object"==typeof e&&null!==e&&(e.name?i.push({name:e.name,generate:!!e.generate}):s={...s,...e})}),0===i.length)return console.warn("[NCCS] No cookies specified for monitoring."),void this._markReady();if(s.timeout=s.timeout||1e3,s.domain=s.domain||c(e.location.hostname),!this.consent)return console.log("[NCCS] Consent is false. Skipping sync."),void this._markReady();const r=this._getUrlOverrides(i),a=Object.keys(r).length>0,l=o(t),u=Date.now();let h=!0;if(l&&!a&&!s.force){const e=parseInt(l,10);!isNaN(e)&&u-e<18e5&&(h=!1)}if(!h)return console.log("[NCCS] Active session within 30 minutes. Skipping sync request."),i.forEach(e=>{const t=o(e.name);t&&(this.values[e.name]=t)}),this._updateSessionCookie(s.domain),this._exposeGlobals(),void this._markReady();const f={cookies:i.map(e=>({name:e.name,value:o(e.name)||null,generate:e.generate})),overrides:r,domain:s.domain};this._sync(f,s)},reset:function(n){const o=c(e.location.hostname);console.log(\`[NCCS] Triggering reset for: \${n||"ALL"}\`),i(t,o);const s=this._getWorkerUrl("/nccs/reset");if(n){i(n,o),delete this.values[n];const e=n.startsWith("_")?n.substring(1):n;delete this.values[e],s.searchParams.set("cookie",n)}else Object.keys(this.values).forEach(e=>{i(e,o),e.startsWith("_")||i("_"+e,o)}),this.values={};try{const e=new XMLHttpRequest;e.open("POST",s.toString(),!0),e.setRequestHeader("Content-Type","application/json"),e.send(JSON.stringify({cookie:n||null}))}catch(e){console.error("[NCCS] Reset network request failed:",e)}this._exposeGlobals()},generate:function(e){console.log(\`[NCCS] Explicitly requesting GUID generation for: \${e}\`),this.init({name:e,generate:!0},{force:!0})},_exposeGlobals:function(){e._nccs=e._nccs||{},Object.entries(this.values).forEach(([t,n])=>{if(e._nccs[t]=n,t.startsWith("_")){const o=t.substring(1);e._nccs[o]=n}})},_markReady:function(){for(this.isReady=!0,console.log("[NCCS] Initialization complete. Triggering callbacks.");this.readyQueue.length>0;){const e=this.readyQueue.shift();try{e()}catch(e){console.error("[NCCS] Callback execution error:",e)}}},_getUrlOverrides:function(t){const n={};try{const o=new URLSearchParams(e.location.search);t.forEach(e=>{const t=\`nccs_\${e.name}\`;if(o.has(t)){const s=o.get(t);s&&(n[e.name]=s)}})}catch(e){console.error("[NCCS] Error parsing URL overrides:",e)}return n},_getWorkerUrl:function(t){let n=e.location.origin;if(document.currentScript&&document.currentScript.src)try{n=new URL(document.currentScript.src).origin}catch(e){}return new URL(t,n)},_updateSessionCookie:function(e){s(t,Date.now().toString(),365,e)},_sync:function(e,t){const n=this._getWorkerUrl("/nccs/sync");if(!0===t.sync){let o=!1,s=null;console.log("[NCCS] Synchronizing cookies with worker (blocking mode)...");try{const i=new XMLHttpRequest;i.open("POST",n.toString(),!1),i.setRequestHeader("Content-Type","application/json");try{i.timeout=t.timeout}catch(e){}i.send(JSON.stringify(e)),200===i.status?(s=JSON.parse(i.responseText),o=!0):console.warn(\`[NCCS] Sync HTTP error: \${i.status}\`)}catch(e){console.warn("[NCCS] Synchronous XHR failed, falling back to asynchronous fetch:",e)}if(o&&s&&s.cookies)return void this._processSyncResponse(s.cookies,t.domain)}this._syncAsync(e,t)},_syncAsync:function(e,t){const n=this._getWorkerUrl("/nccs/sync"),o=new AbortController,s=setTimeout(()=>o.abort(),t.timeout);fetch(n.toString(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e),signal:o.signal}).then(e=>{if(clearTimeout(s),!e.ok)throw new Error(\`HTTP \${e.status}\`);return e.json()}).then(e=>{if(!e||!e.cookies)throw new Error("Invalid response structure");this._processSyncResponse(e.cookies,t.domain)}).catch(t=>{console.error("[NCCS] Asynchronous sync failed:",t),e.cookies.forEach(e=>{e.value&&(this.values[e.name]=e.value)}),this._exposeGlobals(),this._markReady()})},_processSyncResponse:function(e,t){Object.entries(e).forEach(([e,n])=>{n?(s(e,n,365,t),this.values[e]=n):(i(e,t),delete this.values[e])}),this._updateSessionCookie(t),this._exposeGlobals(),this._markReady()}};function o(e){const t=\`; \${document.cookie}\`.split(\`; \${e}=\`);return 2===t.length?decodeURIComponent(t.pop().split(";").shift()):null}function s(t,n,o,s){let i="";if(o){const e=new Date;e.setTime(e.getTime()+24*o*60*60*1e3),i=\`; expires=\${e.toUTCString()}\`}const c=s?\`; domain=\${s}\`:"",r="https:"===e.location.protocol?"; secure":"";document.cookie=\`\${t}=\${encodeURIComponent(n)}\${i}; path=/\${c}; samesite=lax\${r}\`}function i(t,n){const o=n?\`; domain=\${n}\`:"",s="https:"===e.location.protocol?"; secure":"";document.cookie=\`\${t}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/\${o}; samesite=lax\${s}\`}function c(e){if("localhost"===e||"127.0.0.1"===e)return"";const t=e.split(".");if(t.length<=2)return\`.\${e}\`;return new Set(["co","com","org","net","gov","edu"]).has(t[t.length-2])&&t.length>2?\`.\${t.slice(-3).join(".")}\`:\`.\${t.slice(-2).join(".")}\`}e.nccs=n,e.cookie_saver=n}(window);`;
